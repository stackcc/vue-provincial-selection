<template>
    <div class="c_w c_box">
        <div class="c_w c_box item-group" @click="addrClick">
            <div class="item-title c_nowrap">
                <slot name="head">
                    所在区域
                </slot>
            </div>
            <div class="item-content c_nowrap">
                <slot name="content">
        ...
        <div class="model_addr" v-show="addr.isShow">
            ...
            <div class="model-body">
                ...
                <div class="model-hot" v-show="addr.isHot">
                    ...
                    <div class="model-hot-list c_flex">
                        <div class="model-hot-group" v-for="(item,index) in addr.hotList" :key="index"
                             :class="((item.province&&(addr.province==item.province))||(item.city&&(addr.city==item.city)))?'bg_red':''">
                ...
                <div class="model-choose">
                    ...
                    <div class="model-choose-title c_flex">
                        <div :class="addr.province?'':'c_red'" @click="chooseAddress('province')">
                            {{addr.province||'请选择'}}
                        </div>
                        <div v-show="addr.province" :class="addr.city?'':'c_red'" @click="chooseAddress('city')">
                            {{addr.city||'请选择'}}
                        </div>
                        <div v-show="addr.province && addr.city" :class="addr.district?'':'c_red'"
                             @click="chooseAddress('district')">
                            {{addr.district||'请选择'}}
                        </div>
                    </div>
...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            this.addr = Object.assign(this.addr, this.addrtemp);
